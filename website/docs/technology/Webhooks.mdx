...
In order to make OpenCRVS as useful and open as possible to other systems, OpenCRVS publishes the following civil registration events as Webhooks that [mediators](./mediators) can subscribe to.
...
2. Securely host the system `client_id`, `client_secret` and `sha_secret`, returned in the registration process, as environment variables your mediator has access to.
...
4. Respond to the Webhook event as you wish internally, and request further details from OpenHIM to suit your business case. Refer to the [mediator](./mediators) page for examples of raw FHIR resources.
...
Verify that the `topic` value matches the event you're trying to subscribe to.
Respond with the following object:
...
Generate a **SHA256** signature using the `context` array contents at position [0] as the `rawBody` . `signingSecret` is your OpenCRVS `sha_secret`, available to you as an environment variable and generated via the [register](./technicalInteroperability) step. `requestSigningVersion` is `"sha256"`
Compare your signature to the `hmac` prop. If the signatures match, the payload is genuine.
...
| `topic`    | `BIRTH_REGISTERED`                                 | A supported OpenCRVS event type string that will trigger this webhook.                                      |
| `secret`   | `d04aec67-1ef4-467a-a5a8-fa5c89ad71ce`             | Your OpenCRVS `sha_secret`, available to you via the mediator [register](./technicalInteroperability) step. |
...
The Webhook example for birth registration we have introduced here shows how OpenCRVS can integrate with the [MOSIP - the Modular Open Source Identity Platform](https://www.mosip.io/) and share the minimum requireed information necessary to generate a National ID for a child.

We are also grateful to the vendor [Simprints](https://www.simprints.com/) who contributed the following proposed flow to capture biometrics during birth registration using their hardware.

<img
  alt="Simprints integration"
  src={useBaseUrl("assets/technology/Simprints-OpenCRVS.jpg")}
/>
